...
namespace Xuwu.FourDimensionalPortals.Demo
{
    ...
    public class PlayerController : PortalTraveler
    {
        ...
        public bool speedPlatformActive = false;
        ...
        public void OnSwitchView(InputValue value)
        {
            ...
            {
                _portalSystemCameraData.Camera.cullingMask =
                    LayerMask.GetMask("Default", "TransparentFX", "Water", "UI");
                ;
                _portalSystemCameraData.PenetratingViewCullingMask =
                    LayerMask.GetMask("Default", "Ignore Raycast", "Water", "UI");
            }
            else
            {
        ...
        private Vector2 superSpeed;

        public void SuperSpeed()
        {
            if (_moveInput.x > 0)
            {
                superSpeed += new Vector2(Time.deltaTime * 3, 0);
            }
            else
            {
                if (superSpeed.x > 1 && CheckGrounded())
                {
                    superSpeed.x -= Time.deltaTime * 3;
                }
            }

            if (_moveInput.y > 0)
            {
                superSpeed += new Vector2(0, Time.deltaTime * 3);
            }
            else
            {
                if (superSpeed.y > 1 && CheckGrounded())
                {
                    superSpeed.y -= Time.deltaTime * 3;
                }
            }

            superSpeed = Vector2.ClampMagnitude(superSpeed, 4);
        }
        ...
        private void FixedUpdate()
        {
            ...
            _smoothMoveInput = Vector2.SmoothDamp(_smoothMoveInput, (_moveInput * superSpeed) * 6f,
                ref _smoothMoveInputVelocity, .1f);
            ...
            var currToTargetRotation =
                Quaternion.Slerp(transform.rotation, Quaternion.LookRotation(targetForward, WorldUp), .1f) *
            ...
            else
            {
                ...
                if (_smoothMoveInput.magnitude > .1f)
                {
                    ...
                    var lookForward =
                        _rotationHandle.TransformDirection(new Vector3(_smoothMoveInput.x, 0f, _smoothMoveInput.y)
                            .normalized);
                    transform.rotation = Quaternion.Slerp(transform.rotation,
                        Quaternion.LookRotation(lookForward, transform.up), .2f);
            ...
            if (isGrounded)
            {
                if (_isJump)
                {
                    var jumpInitialVelocity =
                        Mathf.Sqrt(-(2f * _currGravitationalAcceleration * _currJumpHeight)) * WorldUp;
            ...
            else
            {
                ...
                velocityChangeX = _smoothMoveInput.x >= 0f
                    ? Mathf.Clamp(velocityChangeX, 0f, _smoothMoveInput.x)
                    : Mathf.Clamp(velocityChangeX, _smoothMoveInput.x, 0f);
                velocityChangeZ = _smoothMoveInput.y >= 0f
                    ? Mathf.Clamp(velocityChangeZ, 0f, _smoothMoveInput.y)
                    : Mathf.Clamp(velocityChangeZ, _smoothMoveInput.y, 0f);

                Rigidbody.AddForce(_rotationHandle.TransformVector(velocityChangeX, 0f, velocityChangeZ) * .5f,
        ...
        private void LateUpdate()
        {
            ...
            if (_isFirstPersonView)
            {
                ...
                _portalSystemCameraData.transform.SetPositionAndRotation(_cameraFollowTarget.position,
            ...
            int hitCount = Physics.RaycastNonAlloc(origin, direction, s_hitResultsBuffer, maxDistance,
            ...
            for (int i = 0; i < hitCount; i++)
            {
                ...
                if (hit.collider.TryGetComponent(out Portal portal) && portal.IsWorkable() &&
                    hit.collider == portal.PlaneMeshCollider)
                {
                    ...
                    hitCount = Physics.RaycastNonAlloc(origin, direction, s_hitResultsBuffer, maxDistance,
                        Physics.DefaultRaycastLayers, QueryTriggerInteraction.Ignore);
                    PhysicsUtils.SortRaycastHits(new Span<RaycastHit>(s_hitResultsBuffer, 0, hitCount));
        ...
        private bool CheckGrounded()
        {
            ...
            int hitCount = Physics.SphereCastNonAlloc(origin, radius, -transform.up, s_hitResultsBuffer, maxDistance,
                Physics.DefaultRaycastLayers, QueryTriggerInteraction.Ignore);
        ...
        protected override void PassThrough(Portal fromPortal, Portal toPortal, Matrix4x4 transferMatrix)
        {
            ...
            var currToTargetRotation =
                Quaternion.LookRotation(targetForward, Vector3.Cross(targetForward, cameraRight)) *
                Quaternion.Inverse(transform.rotation);
            currToTargetRotation.ToAngleAxis(out float angle, out var axis);
            transform.RotateAround(transform.TransformPoint(TransferPivotOffset), axis, angle);
