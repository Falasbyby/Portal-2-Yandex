...
namespace Xuwu.FourDimensionalPortals
{
    ...
    public sealed class Portal : MonoBehaviour
    {
        ...
        public MeshCollider _planeMeshCollider;
        ...
        private bool infoPortalActive = false;
        ...
        [SerializeField] private GameObject[] rayCastCheckPosUpDown;
        [SerializeField] private GameObject[] rayCastCheckPosLeftRight;
        public float rayDistanceUpDown = 1;
        public float rayDistanceLeftRight = 1;

        public void CheckNewPos(Vector3 center, Quaternion targetRotation)
        {
            transform.position = center;
            transform.SetPositionAndRotation(center, targetRotation);
            UpdatePos();
        }
        ...
        {
            RaycastHit[] hitsUpDown = new RaycastHit[rayCastCheckPosUpDown.Length];
            RaycastHit[] hitsLeftRight = new RaycastHit[rayCastCheckPosLeftRight.Length];
            bool HitAll = true;
            for (int i = 0; i < rayCastCheckPosUpDown.Length; i++)
            {
                Vector3 rayOrigin = rayCastCheckPosUpDown[i].transform.position;
                Vector3 rayDirection = rayCastCheckPosUpDown[i].transform.forward;

                if (Physics.Raycast(rayOrigin, rayDirection, out hitsUpDown[i], rayDistanceUpDown))
                {
                }
                else
                {
            ...
            for (int i = 0; i < rayCastCheckPosLeftRight.Length; i++)
            {
                Vector3 rayOrigin = rayCastCheckPosLeftRight[i].transform.position;
                Vector3 rayDirection = rayCastCheckPosLeftRight[i].transform.forward;

                if (Physics.Raycast(rayOrigin, rayDirection, out hitsUpDown[i], rayDistanceLeftRight))
                {
                }
                else
                {
            ...
            if (!HitAll)
            {
                for (int i = 0; i < rayCastCheckPosUpDown.Length; i++)
                {
                    ...
                    if (Physics.Raycast(rayOrigin, rayDirection, out hitsUpDown[i], rayDistanceUpDown))
                    {
                        float distans = rayDistanceUpDown - hitsUpDown[i].distance;
                        if (i == 0)
                        {
                            // Получите угол поворота портала относительно мировых координат
                            Quaternion portalRotation = transform.rotation;

                            // Определите, какая сторона портала считается верхней
                            // Здесь предполагается, что "up" вращенного портала указывает вверх
                            Vector3 portalUp = portalRotation * Vector3.up;

                            // Вычислите смещение вверх или вниз относительно портала
                            Vector3 portalOffset = portalUp * distans;

                            transform.localPosition += portalOffset;
                        }
                        else if (i == 1)
                        {
                            // Получите угол поворота портала относительно мировых координат
                            ...
                            // Определите, какая сторона портала считается верхней
                            // Здесь предполагается, что "up" вращенного портала указывает вверх
                            Vector3 portalUp = portalRotation * Vector3.down;
                    ...
                    if (rayCastCheckPosLeftRight.Length <= 0)
                        return;
                    Vector3 rayOriginSize = rayCastCheckPosLeftRight[i].transform.position;
                    Vector3 rayDirectionSize = rayCastCheckPosLeftRight[i].transform.forward;
                    if (Physics.Raycast(rayOriginSize, rayDirectionSize, out hitsLeftRight[i], rayDistanceLeftRight))
                    {
                        float distans = rayDistanceLeftRight - hitsLeftRight[i].distance;
                        if (i == 0)
                        {
                            // Получите угол поворота портала относительно мировых координат
                            Quaternion portalRotation = transform.rotation;
                            ...
                            // Определите, какая сторона портала считается верхней
                            // Здесь предполагается, что "up" вращенного портала указывает вверх
                            Vector3 portalUp = portalRotation * Vector3.right;
                        ...
                        else if (i == 1)
                        {
                            ...
                            // Определите, какая сторона портала считается верхней
                            // Здесь предполагается, что "up" вращенного портала указывает вверх
                            Vector3 portalUp = portalRotation * Vector3.left;
                    ...
                    Debug.DrawRay(rayOrigin, rayDirection * rayDistanceUpDown, Color.blue);
                    Debug.DrawRay(rayOriginSize, rayDirectionSize * rayDistanceLeftRight, Color.red);
                }
            }
        }
        ...
        private void Update()
        {
            ...
            {
                /*float distans = 3 - hit.distance;
                transform.position = new Vector3(transform.position.x, transform.position.y + distans,
        ...
        public Plane GetPlane(float zOffset = 0f) =>
            new(transform.forward, transform.position + transform.forward * zOffset);

        public Vector4 GetVector4Plane(float zOffset = 0f) => new(transform.forward.x, transform.forward.y,
            transform.forward.z,
            0f - Vector3.Dot(transform.forward, transform.position + transform.forward * zOffset));

        public Matrix4x4 GetTransferMatrix() => _linkedPortal
            ? _linkedPortal.transform.localToWorldMatrix
              * Matrix4x4.Rotate(Quaternion.Euler(0f, 180f, 0f)) * transform.worldToLocalMatrix
            : Matrix4x4.identity;

        public Vector3 TransferDirection(Vector3 direction) => _linkedPortal
            ? _linkedPortal.transform.TransformDirection(
                Quaternion.Euler(0f, 180f, 0f) * transform.InverseTransformDirection(direction))
            : direction;

        public Vector3 TransferPoint(Vector3 position) => _linkedPortal
            ? _linkedPortal.transform.TransformPoint(
                Quaternion.Euler(0f, 180f, 0f) * transform.InverseTransformPoint(position))
            : position;

        public Vector3 TransferVector(Vector3 vector) => _linkedPortal
            ? _linkedPortal.transform.TransformVector(
                Quaternion.Euler(0f, 180f, 0f) * transform.InverseTransformVector(vector))
            : vector;

        public Quaternion TransferRotation(Quaternion rotation) => _linkedPortal
            ? _linkedPortal.transform.rotation
              * Quaternion.Euler(0f, 180f, 0f) * Quaternion.Inverse(transform.rotation) * rotation
            : rotation;
        ...
        public bool LinkPortal(Portal portal)
        {
            ...
            {
                Debug.LogWarning(
                    $"({name}) cannot link to ({portal.name}), because ({portal.name}) has been destroyed.");
                return false;
            }
            ...
            {
                Debug.LogWarning(
                    $"({name}) cannot link to ({portal.name}), because config of ({portal.name}) is missing.");
                return false;
            }
        ...
        public void Validate()
        {
            ...
            _detectionMeshCollider.hideFlags =
                HideFlags.HideInHierarchy | HideFlags.HideInInspector | HideFlags.NotEditable;
            ...
            if (_defaultViewMaterial)
                _defaultViewMaterial.shader = portalSystem.ViewShaderTest;
            else
                _defaultViewMaterial = new Material(portalSystem.ViewShaderTest)
        ...
        private void OnDrawGizmosSelected()
        {
            ...
            localBounds.extents =
                Vector3.Scale(new Vector3(localBounds.extents.x + .05f, localBounds.extents.y + .05f, .5f),
                    _detectionZoneScale);
